...
class CASAuthBackend(CASBackend):
    ...
    def authenticate(self, request, ticket, service):
        ...
        if attributes:
            ...
            logger.warn(
                f"no attributes found in CAS response for username {username} and ticket {ticket}"
            )
        ...
        logger.debug(
            "after configuring user with attributes %s, %s, %s"
    ...
    def configure_user(self, user, request):
        ...
        if attributes:
            try:
                ...
                user.first_name = (
                    first_name[0] if type(first_name) is list else first_name
                )
                user.last_name = last_name[0] if type(last_name) is list else last_name
                user.email = email[0] if type(email) is list else email
                user.save()
                logger.debug(
                    "after saving user with attributes %s, %s, %s"
                    % (user.last_name, user.first_name, user.email)
                )

            except Exception:
                logger.error(
                    "Exception retrieving person attributes, attributes received: {}".format(
                        attributes
                    )
                )
            ...
            try:
                ...
                if member_of:
                    if type(member_of) is list:
                        ...
                        group_ids = list(
                            map(str.strip, member_of.strip("[]").split(","))
                        )

                    if group_ids:
                        request.session["USER_GROUPS"] = group_ids
                        logger.debug(
                            f">>> storing groups for user {user.username} in session: {group_ids}"
                        )
                else:
